...
class PlotTool(QtWidgets.QMainWindow):
    ...
    def init_panel_design(self) -> None:
        ...
        self.plot_panel_design["bgr_color"].setRgb(
            MDC_PANEL_BGR[0], MDC_PANEL_BGR[1], MDC_PANEL_BGR[2]
        )
    ...
    def init_markers(self) -> None:
        ...
        self.set_qspinbox(
            self.marker_1["x"], [-1e6, 1e6], value=0, decimals=6, step=0.01
        )
        ...
        self.set_qspinbox(
            self.marker_2["x"], [-1e6, 1e6], value=0, decimals=6, step=0.01
        )
    ...
    def set_edit_curves_tab(self) -> None:
        ...
        self.set_qspinbox(
            self.ec_offset_x, box_range=[-1e6, 1e6], value=0, decimals=5, step=1e-3
        )
        self.ec_offset_y_lbl = QLabel("y offset:")
        self.ec_offset_y = QDoubleSpinBox()
        self.set_qspinbox(
            self.ec_offset_y, box_range=[-1e6, 1e6], value=0, decimals=4, step=0.1
        )
        ...
        self.set_qspinbox(
            self.ec_scale, box_range=[0, 1e6], value=1, decimals=1, step=1
        )
    ...
    def set_ds_dv_list(self) -> None:
        ...
        for dvi in dv.keys():
            ...
            if isinstance(self.data_browser.data_viewers[dvi], p3d.DataViewer3D):
    ...
    def add_dataset(self) -> None:
        ...
        self.data_items[data_item_lbl]["data_item"] = CustomDataItem(
            x, y, pen=pg.mkPen("k", width=2)
        )
    ...
    def set_pen(self, loading: bool = False) -> None:
        ...
        self.data_items[data_item_lbl]["data_item"].setPen(
            color=cd, width=w, style=style
        )
    ...
    def reset_scaling(self) -> None:
        ...
        self.data_items[data_item_lbl]["data_item"].setData(
            deepcopy(org[0]), deepcopy(org[1])
        )
    ...
    def add_update_annotation(self) -> None:
        ...
        self.annotations[name]["text_item"] = pg.TextItem(
            text=text, anchor=(0, 1), color=clr
        )
    ...
    def save(self) -> None:
        ...
        save_selector_box.setStandardButtons(
        ...
        for key in saved.keys():
            ...
            for keyy in saved[key].keys():
                if keyy == "text":
                    ...
                    self.annotations[key][keyy] = saved[key][keyy]
                clr = saved[key]["color"]
                self.annotations[key]["text_item"] = pg.TextItem(
                    text=saved[key]["text"], anchor=(1, 0), color=clr
                )
