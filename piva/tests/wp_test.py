...
class TestWP:
    ...
    def check_fitting_functions(self):
        ...
        # xps fitting
        ta, ta_erg = wp.sum_XPS([self.xps, self.xps], crop=[-28, -21], 
                                plot=False)
    ...
    def check_arpes_analysis_tools(self):
        ...
        # Lindhard function
        def ek_tb(kx, ky=None, kz=0., t=1.0, mu=0.0):
            return -2 * t * (np.cos(kx) + np.cos(ky)) - mu
        ek_kwargs = {"t": 1., "mu": 0.5}
        kx, qx = np.linspace(-np.pi, np.pi, 5), np.linspace(0., 0.4 * np.pi, 5)
        ky, kz, qy, qz = np.copy(kx), np.copy(kx), np.copy(qx), np.copy(qx)
        _ = wp.get_chi(ek=ek_tb, ek_kwargs=ek_kwargs, kx=kx, qx=qx, ky=ky, 
                         qy=qy, kz=kz, qz=qz, in_plnae=False, crop=False)
        
        # Density wave simulations
        res1D = wp.get_1D_modulation(qx, [1, -1])
        res2D = wp.get_2D_modulation(qx, [np.array([1, 0]), np.array([0, 1])])
        res3D = wp.get_3D_modulation(qx, qz, [np.array([1, 0, 0]), 
                                              np.array([0, 1, 0]), 
                                              np.array([0, 0, 1])])
        
        ____ = wp.get_self_energy_of_FDW((qx), (kx), ek_tb, ek_kwargs, 
                                         np.linspace(-0.5, 0, 5), res1D)
        se2D = wp.get_self_energy_of_FDW((qx, qy), (kx, ky), ek_tb, ek_kwargs, 
                                         np.linspace(-0.5, 0, 5), res2D)
        ____ = wp.get_self_energy_of_FDW((qx, qy, qz), (kx, ky, kz), ek_tb, 
                                         ek_kwargs, np.linspace(-0.5, 0, 5), 
                                         res3D)
    ...
    def check_gap_analysis(self):
        ...
        # extract step
        edc, erg = self.edc.data[150:250], self.edc.x[150:250] - 75. + 4.3
        _ = wp.dec_fermi_div(edc, erg, 10, 0., -0.5)
        _ = wp.deconvolve_resolution(edc, erg, 10.)
    ...
    def check_utilities(self):
        ...
        edc, _ = self.edc.data[150:250], self.edc.x[150:250] - 75. + 4.3
    ...
    def check_image_processing_utilities(self):
        ...
        _ = wp.curvature_1d(self.edc.data, wp.get_step(self.edc.x), nb=5, 
                            rl=2, xaxis=self.edc.x - 75 + 4.3)
    ...
    def check_misc(self):
        ...
        _ = wp.McMillan_Tc()
        _ = wp.dynes_formula(self.edc.x-75+4.3, 1., 0.5, 0.01)
